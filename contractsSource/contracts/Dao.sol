...
contract Dao {
  ...
  modifier onlyMember() {
    require(members[msg.sender].joined != 0, 'Not member');
    _;
  }

  constructor() {
    proposalHandler = new ProposalHandler(address(this));
  }
  ...
  function deposit(uint _amount) external payable {
    ...
    members[msg.sender].joined = block.timestamp;
  ...
  function distributeRewards(uint _proposalId) internal {
    ...
    address[] memory winners = proposalHandler.getWinners(_proposalId);
    ...
    for (uint i = 0; i < winners.length; i++) {
      uint winnerReward = (totalReward *
        winnerPoints[_proposalId][winners[i]]) / allWinnersPoints;
  ...
  function submitProof(
    ...
    proposalHandler._submitVote(key, a, b, c, input, msg.sender);
  }
}
